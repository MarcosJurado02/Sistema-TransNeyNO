...
from core.models import Departamento, Puesto, Sucursal, TipoEmpleado
...
class CatalogAccessTests(TestCase):
	...
	def test_rh_and_ti_can_open_catalogs(self):
		for user in (self.rh_user, self.ti_user):
			...
			self.assertContains(response, 'Tipos de empleado')

	def test_employee_type_catalog_is_available(self):
		self.client.force_login(self.rh_user)

		response = self.client.get(reverse('catalogos:employee-types'))

		self.assertEqual(response.status_code, 200)
		self.assertContains(response, 'Administrativo')
		self.assertTrue(TipoEmpleado.objects.filter(nombre='Sin especificar').exists())
